...
export default function Page() {
    ...
    async function searchCard(userQuery: string) {
        ...
        setCardData(data);
        setCurrentCard(0);
        console.log('cardData.data:', cardData?.data);
        console.log('length:', cardData?.data?.length);
    }
    ...
    return (
        <main>
            ...
            <div className="flex justify-center px-10 items-start relative z-0 -mt-5">
                    ...
                    <CardImage isSearchPage={true} cardData={cardData} currentCard={currentCard} />
                        <div className="flex items-center -mt-22">
                            ...
                            <>
                                ...
                                <h3 className="ml-10 -mt-12 text-large text-white">{currentCard + 1}/{totalCards}</h3>
                                <button className="ml-10 bg-green-600 h-6 mb-15 px-2 rounded-md text-white hover:bg-green-700 -mt-5"
                                    onClick={() => 
                                        setCurrentCard(prev => prev === cardData?.data?.length - 1 ? 0 : prev + 1)}>
                                        Next
                                </button>
                            </>
                    }
